...
interface FeedUsageTableToolbarProps<TData> {
  table: Table<TData>;
  flocks: Array<{ id: string; batchCode: string; currentCount: number }>;
}

export function FeedUsageTableToolbar<TData>({
  table,
  flocks,
}: FeedUsageTableToolbarProps<TData>) {
  ...
  // Feed type options for filtering
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center sm:space-x-2">
        ...
        <div className="flex flex-wrap gap-2">
          {/* Flock Filter */}
          {table.getColumn("flock") && (
            <DataTableFacetedFilter
              column={table.getColumn("flock")}
              title={t('columns.flockId')}

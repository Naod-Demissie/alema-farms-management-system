...
export function FinancialReporting() {
  ...
  const fetchFinancialData = async () => {
    try {
      ...
      const flockResponse = await fetch(`/api/financial/reports/flocks?${queryParams}`);
      if (flockResponse.ok) {
        const flockData = await flockResponse.json();
        setFlockFinancials(flockData);
      }

      // Fetch monthly data
      const monthlyResponse = await fetch(`/api/financial/reports/monthly?${queryParams}`);
